...
export function StrategicAlignmentSection({ dictionary }: { dictionary: LocaleContent }) {
  return (
    <section id="alignment" className="overflow-hidden bg-gradient-to-b from-white via-pmcs-light/45 to-white py-20 md:py-24">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="relative mt-12 rounded-[2.25rem] border border-pmcs-line bg-white/90 p-4 shadow-[0_28px_90px_rgba(43,43,43,0.10)] md:p-6 lg:p-8">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,0.78fr)_minmax(0,1fr)] lg:items-stretch">
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-1">
              {dictionary.alignment.cards.slice(0, 2).map((card) => (
                <article key={card.number} className="group relative overflow-hidden rounded-[1.75rem] border border-pmcs-line bg-white p-6 shadow-sm transition duration-200 ease-out hover:border-pmcs-gold/55 hover:shadow-pmcs motion-reduce:transition-none">
            ...
            <div className="relative flex min-h-full items-center">
              <div className="relative w-full overflow-hidden rounded-[2rem] border border-pmcs-maroon/25 bg-gradient-to-br from-pmcs-maroonDark via-pmcs-maroon to-pmcs-charcoal p-7 text-white shadow-[0_24px_70px_rgba(122,31,43,0.28)] md:p-8">
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-1">
              {dictionary.alignment.cards.slice(2).map((card) => (
                <article key={card.number} className="group relative overflow-hidden rounded-[1.75rem] border border-pmcs-line bg-white p-6 shadow-sm transition duration-200 ease-out hover:border-pmcs-green/45 hover:shadow-pmcs motion-reduce:transition-none">
