import type { LocaleContent } from './types';

export const en: LocaleContent = {
  meta: {
    title: 'PMCS Holdings | Singapore-Led Food Security Platform',
    description: 'PMCS Holdings is a Singapore-led food-security, sustainability, governance, and finance-readiness planning platform under development for a Thailand-to-GCC corridor concept.',
    keywords: ['PMCS Holdings', 'food security', 'sustainability', 'Singapore governance', 'Thailand supply readiness', 'GCC corridor'],
  },
  language: {
    label: 'Language',
    linksHeading: 'Language options',
    options: { en: 'English', th: 'ไทย', ar: 'العربية' },
    switchTo: 'Switch language',
  },
  nav: {
    aria: 'Primary navigation',
    brandName: 'PMCS Holdings',
    logoMark: 'PM',
    homeAria: 'PMCS Holdings home',
    claimRibbon: 'Singapore-led platform under development · Subject to verification, partner validation and final documentation.',
    logoAlt: 'PMCS Holdings placeholder logo',
    menuOpen: 'Menu',
    menuClose: 'Close menu',
    items: [
      { label: 'Overview', href: 'overview' },
      { label: 'Strategic Alignment', href: 'alignment' },
      { label: 'Platform Ecosystem', href: 'ecosystem' },
      { label: 'ICCR Framework', href: 'iccr' },
      { label: 'Thailand Supply Base', href: 'supply' },
      { label: 'Capital & Partnerships', href: 'capital' },
      { label: 'Governance', href: 'governance' },
    ],
    contact: 'Contact',
    mega: {
      trigger: 'Explore',
      overlayLabel: 'Close platform menu overlay',
      groups: [
        {
          eyebrow: 'Platform view',
          heading: 'Concept overview',
          description: 'Navigate the verification-led platform narrative and its planned institutional positioning.',
          links: [
            { label: 'Overview', href: 'overview', description: 'Singapore-led platform concept under development.' },
            { label: 'Strategic Alignment', href: 'alignment', description: 'Governance planning, Thailand readiness and GCC priorities.' },
            { label: 'Platform Ecosystem', href: 'ecosystem', description: 'Structured corridor concept with cautious boundaries.' },
          ],
        },
        {
          eyebrow: 'Readiness layers',
          heading: 'Sustainability and supply',
          description: 'Review planning layers that remain subject to validation, approvals and final documentation.',
          links: [
            { label: 'ICCR Framework', href: 'iccr', description: 'Planning pillars for sustainability-linked corridor development.' },
            { label: 'Thailand Supply Base', href: 'supply', description: 'Production readiness planning subject to partner verification.' },
          ],
        },
        {
          eyebrow: 'Institutional controls',
          heading: 'Capital and governance',
          description: 'Access claim-controlled information on capital-readiness planning, potential counterparties and compliance positioning.',
          links: [
            { label: 'Capital & Partnerships', href: 'capital', description: 'Planning concepts only, without investment solicitation.' },
            { label: 'Governance', href: 'governance', description: 'Compliance-first positioning for a platform under development.' },
            { label: 'Contact', href: 'contact', description: 'Informational contact section for alignment discussions.' },
          ],
        },
      ],
      cards: [
        {
          eyebrow: 'Phase 1.2 preview',
          title: 'Institutional corridor planning',
          description: 'Placeholder cards indicate future visual modules for Singapore governance, Thailand readiness and GCC engagement.',
          placeholderAria: 'Placeholder preview modules',
          placeholderLabels: ['Governance', 'Readiness', 'Engagement'],
        },
        {
          eyebrow: 'Claim control',
          title: 'Subject to verification',
          description: 'Menu copy remains informational and does not claim secured financing, issued carbon credits or binding offtake arrangements.',
          placeholderAria: 'Verification placeholder modules',
          placeholderLabels: ['Review', 'Approval', 'Documentation'],
        },
      ],
    },
  },
  hero: {
    eyebrow: 'Singapore-led platform under development',
    title: 'Institutional readiness for a Singapore-led food-security corridor concept.',
    lead: 'PMCS Holdings is being structured as a cautious platform concept intended to align Singapore-led governance, Thailand-based production readiness, sustainability controls and GCC food-security engagement through a verification-led corridor approach.',
    primaryCta: 'Explore platform concept',
    secondaryCta: 'Contact section',
    note: 'Informational website only. The content presents PMCS Holdings as a platform-readiness concept under development and does not represent an agriculture seller, commodity exporter, investment product, completed operating platform, secured financing vehicle or binding offtake arrangement.',
    imageAlt: 'Abstract platform-readiness card for a Singapore-led institutional food-security corridor concept',
    imageCaption: 'Platform readiness placeholder; reviewed imagery may be added in a later phase.',
    visual: {
      heading: 'Platform readiness view',
      subheading: 'Singapore-led coordination layer',
      modules: [
        { label: 'Governance', detail: 'Documentation discipline' },
        { label: 'Thailand readiness', detail: 'Subject to validation' },
        { label: 'Sustainability / ICCR', detail: 'Controls under development' },
        { label: 'GCC engagement', detail: 'Subject to final documentation' },
      ],
      disclosure: 'Conceptual readiness map only',
      statusLabel: 'Planning visual',
    },
  },
  alignment: {
    eyebrow: 'Strategic alignment',
    title: 'A coordination platform concept linking disciplined governance with corridor readiness.',
    body: 'PMCS Holdings is being structured as a Singapore-led planning framework intended to coordinate governance discipline, Thailand-based production readiness, sustainability and ICCR-aligned controls, GCC food-security engagement, and capital-readiness workstreams. Each pathway remains under development and subject to verification, partner validation, applicable approvals and final commercial documentation.',
    modalCloseLabel: 'Close strategic alignment details',
    modalKeyPointsLabel: 'Key points',
    modalClaimLabel: 'Claim-control note',
    cards: [
      {
        number: '01',
        eyebrow: 'Governance',
        title: 'Singapore Governance Platform',
        body: 'A coordination layer for documentation discipline and claim-controlled institutional communication.',
        cta: 'View details',
        imageAlt: 'Strategic alignment card visual for a Singapore governance platform planning concept',
        cardImage: '/assets/strategic-alignment/pmcs-singapore-governance-platform-card.webp',
        modalImage: '/assets/strategic-alignment/pmcs-singapore-governance-platform-modal.webp',
        subtitle: 'A planning concept for documentation discipline and institutional oversight.',
        expandedBody: 'This workstream positions Singapore as the intended coordination layer for governance discipline, evidence review and public-claim controls. It is a platform-readiness concept under development and depends on verification, partner validation, applicable approvals and final documentation before any formal representation is made.',
        keyPoints: [
          'Intended oversight structure for documentation, controls and review gates.',
          'Designed to keep public communication verification-led and claim-controlled.',
          'Subject to partner validation, applicable approvals and final documentation.',
        ],
        claimNote: 'Informational only. This does not claim regulatory approval granted, confirmed strategic partners, completed operations or a binding governance mandate.',
      },
      {
        number: '02',
        eyebrow: 'Readiness',
        title: 'Thailand Production Readiness',
        body: 'A readiness pathway for production coordination, traceability and partner validation.',
        cta: 'View details',
        imageAlt: 'Strategic alignment card visual for Thailand production readiness planning',
        cardImage: '/assets/strategic-alignment/pmcs-thailand-production-readiness-card.webp',
        modalImage: '/assets/strategic-alignment/pmcs-thailand-production-readiness-modal.webp',
        subtitle: 'A readiness pathway for supply planning that remains subject to verification.',
        expandedBody: 'The Thailand production layer is framed as a readiness pathway for future evaluation of production discipline, supply coordination, traceability logic and operating controls. The concept remains under development and should not be read as completed full-scale operations or validated capacity.',
        keyPoints: [
          'Site-level readiness, operating controls and documentation remain subject to verification.',
          'Partner validation is required before production capability is represented as confirmed.',
          'Final commercial documentation would be required for any formal supply commitment.',
        ],
        claimNote: 'This planning concept does not imply completed full-scale operations, binding offtake, guaranteed supply volumes or confirmed market access.',
      },
      {
        number: '03',
        eyebrow: 'Sustainability',
        title: 'Sustainability & ICCR Discipline',
        body: 'An ICCR-aligned planning framework for sustainability discipline and future verification.',
        cta: 'View details',
        imageAlt: 'Strategic alignment card visual for sustainability and ICCR discipline under development',
        cardImage: '/assets/strategic-alignment/pmcs-iccr-sustainability-discipline-card.webp',
        modalImage: '/assets/strategic-alignment/pmcs-iccr-sustainability-discipline-modal.webp',
        subtitle: 'A verification-led discipline for sustainability controls and ICCR-aligned planning.',
        expandedBody: 'This pathway organizes sustainability and ICCR-aligned controls as a documentation and verification discipline. It supports responsible review of potential environmental claims while keeping certification, carbon-credit and approval language subject to evidence, applicable processes and final documentation.',
        keyPoints: [
          'Sustainability positioning remains subject to verification and documentation.',
          'No certified net-zero status, issued carbon credits or verified emissions reductions are claimed.',
          'Applicable approvals and final documentation are required before any formal claim is made.',
        ],
        claimNote: 'Informational only. The pathway does not represent certified net zero, issued carbon credits, completed certification or regulatory approval granted.',
      },
      {
        number: '04',
        eyebrow: 'GCC corridor',
        title: 'GCC Food-Security Corridor',
        body: 'A corridor-readiness concept for structured GCC food-security discussion.',
        cta: 'View details',
        imageAlt: 'Strategic alignment card visual for a GCC food-security corridor planning concept',
        cardImage: '/assets/strategic-alignment/pmcs-gcc-food-security-corridor-card.webp',
        modalImage: '/assets/strategic-alignment/pmcs-gcc-food-security-corridor-modal.webp',
        subtitle: 'A corridor-planning concept for potential food-security alignment discussions.',
        expandedBody: 'The GCC corridor workstream is presented as a planning concept for evaluating food-security relevance, counterparties, documentation requirements and potential market pathways. Any engagement remains subject to partner validation, applicable approvals and final commercial documentation.',
        keyPoints: [
          'Counterparty engagement and market pathways remain under development.',
          'Market access, approvals and offtake would require validation and final documentation.',
          'The concept is intended for disciplined alignment discussion, not completed trade representation.',
        ],
        claimNote: 'This does not imply confirmed strategic partners, binding offtake, regulatory approval granted or guaranteed market access.',
      },
      {
        number: '05',
        eyebrow: 'Capital',
        title: 'Capital-Readiness Pathway',
        body: 'A documentation-led pathway for diligence, governance review and future institutional discussion.',
        cta: 'View details',
        imageAlt: 'Strategic alignment card visual for a capital-readiness pathway under development',
        cardImage: '/assets/strategic-alignment/pmcs-capital-readiness-pathway-card.webp',
        modalImage: '/assets/strategic-alignment/pmcs-capital-readiness-pathway-modal.webp',
        subtitle: 'A documentation-first pathway for future capital and partnership review.',
        expandedBody: 'The capital-readiness layer is limited to documentation discipline, diligence preparation, governance review and partner-readiness milestones. It is not active fundraising, does not solicit investment and does not indicate that financing, returns or counterparties have been secured.',
        keyPoints: [
          'Readiness work focuses on diligence materials, review gates and final documentation.',
          'No secured financing, active fundraising or guaranteed returns are represented.',
          'Any future capital or partnership pathway would remain subject to verification and applicable approvals.',
        ],
        claimNote: 'Informational only and not an investment solicitation. Final documentation is required and no financing, returns or binding partnership outcome is represented as secured.',
      },
    ],
    controlsLabel: 'Strategic alignment claim controls',
    controls: ['Subject to verification', 'Partner validation required', 'Regulatory approval where applicable', 'Final commercial documentation required'],
  },

  ecosystem: {
    eyebrow: 'Platform ecosystem',
    title: 'A coordinated platform architecture being structured for corridor readiness.',
    body: 'PMCS Holdings is presented as a developing planning-and-coordination platform concept intended to connect governance discipline, Thailand-based supply readiness, ICCR-aligned sustainability controls, finance-readiness planning and GCC food-security engagement. The section describes a planning layer only and does not claim completed operations, secured financing, issued carbon credits or binding offtake.',
    selectorAria: 'Platform ecosystem topic selector',
    openDetailsLabel: 'Open details',
    closeLabel: 'Close platform ecosystem detail',
    items: [
      {
        label: '01',
        eyebrow: 'Coordinating layer',
        title: 'PMCS Platform Layer',
        short: 'Planning layer for corridor readiness.',
        body: 'A verification-led planning concept under development to coordinate claims, counterparties, documentation and readiness workstreams before any final commitments are represented.',
        image: '/assets/platform-ecosystem/pmcs-platform-ecosystem-coordinating-layer.webp',
        imageAlt: 'Conceptual PMCS platform layer visual for planning and coordination',
      },
      {
        label: '02',
        eyebrow: 'Governance discipline',
        title: 'Governance and claim control',
        short: 'Cautious disclosures and approval boundaries.',
        body: 'Governance planning keeps language cautious, documentation-led and subject to applicable approvals, partner validation and final documentation.',
        image: '/assets/platform-ecosystem/pmcs-governance-claim-control-platform-ecosystem.webp',
        imageAlt: 'Governance and claim-control visual for the PMCS platform ecosystem',
      },
      {
        label: '03',
        eyebrow: 'Supply readiness',
        title: 'Thailand supply readiness',
        short: 'Traceability and partner validation planning.',
        body: 'Thailand-based supply readiness is framed as planned production, traceability and validation work that remains subject to verification and counterparties.',
        image: '/assets/platform-ecosystem/pmcs-thailand-supply-readiness-platform-ecosystem.webp',
        imageAlt: 'Thailand supply-readiness visual for the PMCS platform ecosystem',
      },
      {
        label: '04',
        eyebrow: 'Sustainability controls',
        title: 'ICCR / sustainability controls',
        short: 'Controls without carbon-credit overclaims.',
        body: 'ICCR-aligned controls are intended to support sustainability discipline where applicable, without claiming issued carbon credits, certified net zero or completed approvals.',
        image: '/assets/platform-ecosystem/pmcs-iccr-sustainability-controls-platform-ecosystem.webp',
        imageAlt: 'ICCR sustainability-controls visual for the PMCS platform ecosystem',
      },
      {
        label: '05',
        eyebrow: 'Finance readiness',
        title: 'Capital readiness pathway',
        short: 'Diligence and documentation sequencing.',
        body: 'Capital readiness is described as a pathway for diligence, structuring inputs and documentation preparation, not as secured financing or active fundraising.',
        image: '/assets/platform-ecosystem/pmcs-capital-readiness-pathway-platform-ecosystem.webp',
        imageAlt: 'Capital-readiness pathway visual for the PMCS platform ecosystem',
      },
      {
        label: '06',
        eyebrow: 'Corridor engagement',
        title: 'GCC engagement planning',
        short: 'Food-security alignment planning.',
        body: 'GCC engagement is positioned as food-security corridor planning, with market access, offtake and counterparties subject to partner validation and final documentation.',
        image: '/assets/platform-ecosystem/pmcs-gcc-food-security-corridor-planning-platform-ecosystem.webp',
        imageAlt: 'GCC food-security corridor planning visual for the PMCS platform ecosystem',
      },
    ],
  },
  iccr: {
    eyebrow: 'ICCR framework',
    title: 'Integrated Carbon Circular Ranching as a planning layer for intended operating discipline.',
    body: 'ICCR — Integrated Carbon Circular Ranching — is being developed as a framework intended to organize sustainability-linked controls across carbon management, resource circularity and finance-readiness workstreams, subject to measurement, verification and final documentation.',
    frameworkAria: 'CSS-only ICCR framework diagram connecting four planning pillars around the central ICCR framework',
    frameworkLabel: 'Planning framework',
    frameworkTitle: 'ICCR Framework',
    frameworkBody: 'A technical coordination layer intended to connect operating inputs, transformation processes, output treatment and carbon-finance readiness without claiming certified results.',
    pillars: [
      { title: 'Carbon Input Control', body: 'Defines planned controls for feed, energy, water and operating inputs so sustainability assumptions can be documented and tested where applicable.' },
      { title: 'Carbon Transformation', body: 'Organizes resource circularity planning around manure, biomass, nutrients and process flows that remain subject to technical validation.' },
      { title: 'Carbon Output Neutralization', body: 'Frames emissions, waste and residual-output pathways for MRV development, mitigation planning and partner review before any claims are made.' },
      { title: 'Carbon Finance Integration', body: 'Prepares diligence inputs for sustainability-linked finance concepts, with any credits, finance outcomes or ESG claims subject to verification, partner validation and applicable approvals.' },
    ],
    noteLabel: 'MRV and claim-control discipline',
    note: 'Carbon, MRV, credit, finance and ESG statements require measurement, partner validation, independent verification where applicable, regulatory approval where required and final documentation. The framework does not claim certified net zero, issued carbon credits or verified emission reductions.',
  },
  supply: {
    eyebrow: 'Thailand supply base',
    title: 'A developing supply-readiness layer for verification-led corridor planning.',
    body: 'Thailand-based supply readiness is being structured as an operating planning layer intended to organize production discipline, supply coordination, documentation logic and staged partner validation.',
    imageAlt: 'Planned visual card reserving space for future Thailand supply-base documentation imagery',
    imageCaption: 'Reserved for future Thailand production-base visual',
    plannedVisual: {
      eyebrow: 'Planned visual',
      title: 'Reserved for future Thailand production-base visual',
      body: 'Documentary imagery of farm readiness, supply coordination, traceability context and documentation discipline.',
      status: 'Reserved for imagery subject to verification',
    },
    readinessCards: [
      { title: 'Production readiness', body: 'Planned operating structure and readiness discipline.' },
      { title: 'Farm / supply coordination', body: 'Coordinated supply workflow subject to partner validation.' },
      { title: 'Traceability & documentation', body: 'Documentation and record logic for future verification.' },
      { title: 'Partner validation / quality pathway', body: 'Quality pathway prepared for staged partner review.' },
    ],
    note: 'Supply readiness remains subject to verification, documentation, partner validation and applicable approvals.',
  },
  capital: {
    eyebrow: 'Capital and partnerships',
    title: 'Capital-readiness concept for disciplined partnership planning.',
    body: 'PMCS describes a planning architecture only: pathways remain under development and subject to verification, partner validation, applicable approvals and final documentation.',
    dashboard: {
      kicker: 'Capital Readiness Control Panel',
      title: 'Planning controls before any capital or partnership step is represented as complete.',
      subtitle: 'Planning architecture only',
      statusClusterLabel: 'Capital readiness status cluster',
      statusCluster: ['Planning', 'Documentation required', 'Subject to validation', 'Approval where applicable'],
      tiles: [
        { title: 'Capital readiness', status: 'Planning', description: 'Readiness concept for capital-related review, subject to verification and final documentation.' },
        { title: 'Partnership pathway', status: 'Under review', description: 'Engagement pathway for potential partners, with partner validation required.' },
        { title: 'Documentation discipline', status: 'Documentation required', description: 'Records, terms and evidence are required before any formal representation.' },
        { title: 'Governance controls', status: 'Approval where applicable', description: 'Governance review and applicable approvals remain part of the planning pathway.' },
        { title: 'Validation gates', status: 'Subject to validation', description: 'Progression depends on verification, counterparties and final documentation.' },
      ],
    },
    pathway: {
      eyebrow: 'Readiness pathway',
      title: 'Concept to final documentation review, presented as an uncompleted planning pathway.',
      note: 'Each stage is a validation gate and should not be read as secured financing, a current capital raise, binding investment, completed offtake or approved regulatory status.',
      ariaLabel: 'Capital readiness planning pathway',
      steps: ['Concept', 'Documentation', 'Partner validation', 'Governance review', 'Final documentation review'],
    },
    claimControl: 'Not an investment solicitation. Capital pathways remain subject to verification, partner validation, applicable approvals and final documentation.',
  },
  governance: {
    eyebrow: 'Governance discipline',
    title: 'A claim-control layer for institutional representation before final commitments.',
    body: 'Governance is positioned as the control layer that keeps platform language disciplined, documented and subject to validation before approvals, partner references or formal commitments are represented as complete.',
    frameworkAria: 'Governance control architecture showing claim control, documentation discipline, partner validation, approval boundaries and public representation controls',
    visualCaption: 'Reserved for future governance visual: documentation discipline, validation workflow and approval-control context.',
    visualStatus: 'Validation workflow',
    central: {
      eyebrow: 'Control architecture',
      title: 'Governance Control Layer',
      body: 'Coordinates cautious representation, documentation discipline and approval where applicable.',
    },
    nodes: [
      { title: 'Claim control', body: 'Statements remain bounded by evidence and are not represented as complete unless documented.' },
      { title: 'Documentation discipline', body: 'Materials are organized around source records, review status and final documentation boundaries.' },
      { title: 'Partner validation', body: 'Partner references and pathway language require authorization, validation and documented context.' },
      { title: 'Approval boundaries', body: 'Regulatory, sustainability and commercial references remain subject to approval where applicable.' },
      { title: 'Public representation', body: 'Website language is maintained as informational, cautious and subject to final documentation.' },
    ],
    checklistTitle: 'Governance control checklist',
    controls: [
      'Claims are represented only where documented.',
      'Capital pathways remain non-solicitation unless formalized.',
      'Supply readiness remains subject to validation.',
      'Sustainability and carbon statements require MRV and applicable approvals.',
      'Partner references require authorization and final documentation.',
    ],
    disclosureLabel: 'Claim-control note',
    disclosure: 'Governance statements are presented as planning and control principles only and remain subject to documentation, validation and applicable approvals.',
  },
  contact: {
    eyebrow: 'Contact gateway',
    title: 'Prepare a structured informational discussion.',
    body: 'PMCS Holdings provides an informational pathway for appropriate institutional inquiry through a documentation-led exchange, subject to verification and final documentation where applicable.',
    readiness: [
      { title: 'Documentation-led discussion', body: 'Information exchange should remain anchored to records and review status.' },
      { title: 'Partner validation where applicable', body: 'Counterparty references require suitable confirmation before use.' },
      { title: 'Non-solicitation communication', body: 'The contact pathway is informational and not an investment offering.' },
    ],
    form: {
      aria: 'Static contact readiness form shell for institutional inquiries',
      eyebrow: 'Inquiry preparation',
      title: 'Contact form shell',
      intro: 'Use this visual form shell to frame the type of alignment discussion that may be supported after final routing is confirmed.',
      fields: {
        name: { label: 'Name', placeholder: 'Your name' },
        organisation: { label: 'Organisation', placeholder: 'Organisation or institution' },
        email: { label: 'Email', placeholder: 'Professional email' },
        inquiryType: { label: 'Inquiry type', placeholder: 'Select inquiry type' },
        message: { label: 'Message', placeholder: 'Briefly outline the discussion context. Do not include sensitive information.' },
      },
      inquiryOptions: ['Potential alignment discussion', 'Documentation review', 'Supply / platform inquiry', 'General institutional inquiry'],
      submitLabel: 'Contact channel to be connected',
      submitNote: 'Submission channel will be connected after final contact details and routing are confirmed.',
    },
    disclosure: 'Contact discussions are informational only and do not represent investment solicitation, binding commitment, approval-granted financing, guaranteed supply, regulatory approval or completed partnership.',
  },
  footer: {
    brandDescription: 'Singapore-led food-security, sustainability, governance and finance-readiness planning platform concept for Thailand-to-GCC corridor planning. The platform remains under development and subject to verification, partner validation, applicable approvals and final documentation.',
    navigationLabel: 'Footer platform navigation',
    platformHeading: 'Platform',
    governanceHeading: 'Governance and claim control',
    governanceItems: [
      'Subject to verification.',
      'Partner validation required.',
      'Regulatory approval where applicable.',
      'Final commercial documentation required.',
    ],
    staticExportHeading: 'Static export readiness',
    staticExport: 'This website is maintained as a lightweight static-export-ready Next.js site for HostGator shared hosting.',
    legalItems: [
      'Informational website only.',
      'No investment solicitation.',
      'No secured financing, issued carbon credits, regulatory approvals, completed operations or binding offtake claims.',
    ],
    rights: 'All rights reserved.',
  },
};
